...
-- 04-DEC-2022    LAFS-10141                 Praveen k         Added process_close_po_inbound
--------------------------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------
-- Purpose  : Call Willow's API to close pos in bound
--
--
-- Modification history
--
-- Version Date   Log Number                 Changed By        Description/Reason for change
-- -----------    ------------               ----------        -------------------------------------------------------------------------------------------
-- 04-DEC-2022    LAFS-10141                 Praveen k         Initial
----------------------------------------------------------------------------------------------------------------------------------------------------------
 PROCEDURE process_close_po_inbound(pi_doc_id IN NUMBER) IS
  BEGIN
    accounting_integration.closePurchaseOrderInbound(pi_doc_id);
  EXCEPTION
    WHEN OTHERS THEN
      wms.raise_program_alert ('acct_app.transaction_api_wrapper.process_close_po_inbound', 'W2K-99999', 'ERROR', SQLCODE, SQLERRM (SQLCODE));
      RAISE;
  END process_close_po_inbound;

---------------------------------------------------------------------------------------------
-- Purpose  : Call Willow's API to get lease type
--
--
-- Modification history
--
-- Version Date   Log Number                 Changed By        Description/Reason for change
-- -----------    ------------               ----------        -------------------------------------------------------------------------------------------
-- 15-DEC-2022    LAFS-10231                 Mahbubul k         Initial
-- 15-DEC-2022    LAFS-10230                 Mahbubul k         Bug fixing for client purhcase
----------------------------------------------------------------------------------------------------------------------------------------------------------
 PROCEDURE get_lease_type(v_unit_no IN fleet_masters.unit_no%type,  v_lease_type OUT VARCHAR2 ) IS
  v_fms_id fleet_masters.fms_id%type;
  BEGIN
    select fms_id into v_fms_id from fleet_Masters fms where fms.unit_No = v_unit_no;
    v_lease_type := fl_maint_label.get_lease_type(v_fms_id);

    EXCEPTION
      WHEN OTHERS THEN
        wms.raise_program_alert ('acct_app.transaction_api_wrapper.get_lease_type', 'W2K-99999', 'ERROR', SQLCODE, SQLERRM (SQLCODE));
        RAISE;
  END get_lease_type;
