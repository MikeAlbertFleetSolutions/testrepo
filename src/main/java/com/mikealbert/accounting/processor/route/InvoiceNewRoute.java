...
@Component
public class InvoiceNewRoute extends BaseRoute{
	...
	public void configure() throws Exception {
		...
		errorHandler(deadLetterChannel(String.format("jms:%s.%s.%s", qPrefix, QueueEnum.INVOICE_AP_NEW.getName(), "DEAD"))
				.useOriginalMessage()				
		...
		from(String.format("jms:%s.%s", qPrefix, QueueEnum.INVOICE_AP_NEW.getName()))
		.to("bean:invoiceInitializationProcessor")		
		.to("bean:invoiceReadProcessor")
		.to("bean:invoiceNewProcessor");
	}
