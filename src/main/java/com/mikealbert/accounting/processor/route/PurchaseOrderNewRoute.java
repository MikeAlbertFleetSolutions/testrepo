...
@Component
public class PurchaseOrderNewRoute extends BaseRoute {	
	...
	public void configure() throws Exception {		  		
		...
		errorHandler(deadLetterChannel(String.format("jms:%s.%s.%s", qPrefix, QueueEnum.PO_NEW.getName(), "DEAD"))
				.useOriginalMessage()				
		...
		from(String.format("jms:%s.%s?concurrentConsumers=1", qPrefix, QueueEnum.PO_NEW.getName()))
		  .to("bean:purchaseOrderReadProcessor")
		  .to("bean:purchaseOrderNewProcessor");
	}
