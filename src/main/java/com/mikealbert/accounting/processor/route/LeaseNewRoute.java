...
@Component
public class LeaseNewRoute extends BaseRoute {		
	...
	public void configure() throws Exception {
		...
		errorHandler(deadLetterChannel(String.format("jms:%s.%s.%s", qPrefix, QueueEnum.LEASE_NEW.getName(), "DEAD"))
				.useOriginalMessage()
		...
		from(String.format("jms:%s.%s?concurrentConsumers=4", qPrefix, QueueEnum.LEASE_NEW.getName()))
		.to("bean:leaseNewReadProcessor")
		.to("bean:leaseNewProcessor");		
	}
