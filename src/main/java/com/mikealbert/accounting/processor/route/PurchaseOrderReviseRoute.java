...
@Component
public class PurchaseOrderReviseRoute extends BaseRoute {	
	...
	public void configure() throws Exception {		  		
		...
		errorHandler(deadLetterChannel(String.format("jms:%s.%s.%s", qPrefix, QueueEnum.PO_REVISE.getName(), "DEAD"))		
				.useOriginalMessage()				
		...
		from(String.format("jms:%s.%s?concurrentConsumers=1", qPrefix, QueueEnum.PO_REVISE.getName()))
		  .to("bean:purchaseOrderReviseProcessor");
	}
