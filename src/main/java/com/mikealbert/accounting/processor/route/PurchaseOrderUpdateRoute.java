...
@Component
public class PurchaseOrderUpdateRoute extends BaseRoute {	
	...
	public void configure() throws Exception {		  		
		...
		errorHandler(deadLetterChannel(String.format("jms:%s.%s.%s", qPrefix, QueueEnum.PO_UPDATE.getName(), "DEAD"))
				.useOriginalMessage()				
		...
		from(String.format("jms:%s.%s?concurrentConsumers=1", qPrefix, QueueEnum.PO_UPDATE.getName()))		
		  .to("bean:purchaseOrderUpdateProcessor");
	}
